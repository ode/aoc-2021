...
lowpoints l = sum $ zipWith (\(a,b) (c,d) -> if a&&c then b+1 else 0) (concatMap g l) 
...
bfs (q:qs) n m
    ...
    | otherwise = bfs (qs++neighbours) (n+1) (adjust (\(n,_)->(n,True)) q m) where
        neighbours = filter valid $ let (x, y) = q in [(x+1,y), (x-1,y), (x,y+1), (x,y-1)]

main = readFile "9.txt" >>= print . lowpoints . map (map (read . (:[]))) . lines
main2 = do
    m <- readFile "9.txt" >>= pure . map (map (read . (:[]))). lines :: IO [[Int]]
    let m' = fromList [((i,j), (a, False)) | (i,b) <- zip [1..] m, (j,a) <- zip [1..] b]
    print $ product . take 3 . sortOn negate . fst $ foldl (\(n,m) q ->
        let (n',m') = bfs [q] 0 m in (n':n, m')) ([], m') [(i, j) | i <- [1..100], j <- [1..100]]